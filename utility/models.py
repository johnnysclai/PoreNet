...
class HardNetSTN(nn.Module):
	def __init__(self, in_c=1, size=32, dropout=0.3, loss='triplet_margin'):
		super(HardNetSTN, self).__init__()
		self.features = nn.Sequential(
			nn.Conv2d(in_c, 32, kernel_size=3, padding=1, bias=False),
		...
		self.localization = nn.Sequential(
			nn.Conv2d(in_c, 8, kernel_size=7),
